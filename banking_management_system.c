    ...
    while (fscanf(old,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d",&add.acc_no,add.name,&add.dob.day,&add.dob.month,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.day,&add.deposit.month,&add.deposit.year)!=EOF)
   {
            ...
            {   test=1;
                ...
                else
                {
                    ...
                    scanf("%f",&transaction.amt);
                    if(add.amt-transaction.amt>100){
                    add.amt-=transaction.amt;
                    printf("Balance is : %f\n",add.amt);
                    fprintf(newrec,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d\n",add.acc_no,add.name,add.dob.day,add.dob.month,add.dob.year,add.age,add.address,add.citizenship,add.phone,add.acc_type,add.amt,add.deposit.day,add.deposit.month,add.deposit.year);
                    printf("\n\nWithdrawn successfully!");}
                    else{
                    	printf("An amount of 100 should be maintained. So Withdrawl of %f is not possible\n",transaction.amt);
                    	transact();
					}
                }
            }
...
void close(void)
{
    printf("\n\n\n\nBanking Management System is closed!");
}

int main()
{
    char pass[10],password[10]="rishitha";
    int i=0;
